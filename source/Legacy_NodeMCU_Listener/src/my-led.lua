...
local showColor = function(operatorR, operatorG, operatorB, stageR, stageG, stageB)
    if lastColors and lastColors[1] == operatorR and lastColors[2] == operatorG and lastColors[3] == operatorB and
       lastColors[4] == stageR and lastColors[5] == stageG and lastColors[6] == stageB then return end
    lastColors = {operatorR, operatorG, operatorB, stageR, stageG, stageB}
    --
    ...
    if MySettings.operatorNumberOfWs2812Lights() + MySettings.stageNumberOfWs2812Lights() > 0 then
        ...
        if MySettings.operatorWs2812Type() == Ws2812Types.GRB then
            data = string.rep(string.char(operatorG, operatorR, operatorB), MySettings.operatorNumberOfWs2812Lights())
        elseif MySettings.operatorWs2812Type() == Ws2812Types.RGB then
            data = string.rep(string.char(operatorR, operatorG, operatorB), MySettings.operatorNumberOfWs2812Lights())
        end
        if MySettings.stageWs2812Type() == Ws2812Types.GRB then
            data = data .. string.rep(string.char(stageG, stageR, stageB), MySettings.stageNumberOfWs2812Lights())
        elseif MySettings.stageWs2812Type() == Ws2812Types.RGB then
            data = data .. string.rep(string.char(stageR, stageG, stageB), MySettings.stageNumberOfWs2812Lights())
        end
        ws2812.write(data)
    end
