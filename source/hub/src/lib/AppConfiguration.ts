...
const supportedMixerSelections = ["atem", "obs", "vmix"]
...
export class AppConfiguration extends Configuration {
    ...
    setMixerSelection(mixerSelection: string) {
        if (!supportedMixerSelections.includes(mixerSelection)) {
            console.warn(`Unsupported mixer "${mixerSelection}" was requested. Falling back to "obs".`)
            mixerSelection = "obs"
        }
